...
class CraftingTableIIEntityRenderer(arg: BlockEntityRendererFactory.Context) :
    ...
    private val texture = SpriteIdentifier(
        PlayerScreenHandler.BLOCK_ATLAS_TEXTURE, Identifier(CraftingTableIIMod.MOD_ID + ":block/craftingtableii")
    )
    ...
    override fun render(
        ...
        val blockState = if (entity.hasWorld()) entity.cachedState
        else (CraftingTableIIMod.BLOCK.defaultState.with(
            Properties.HORIZONTAL_FACING, Direction.SOUTH
        ))
        val lightAbove = if (entity.hasWorld()) WorldRenderer.getLightmapCoordinates(
            entity.world, entity.cachedState, entity.pos.up()
        )
        else light
